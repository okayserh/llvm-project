...
SDNode* T8xxDAGToDAGISel::getGlobalBaseReg() {
...
bool T8xxDAGToDAGISel::SelectADDRri(SDValue Addr,
...
bool T8xxDAGToDAGISel::SelectADDRrr(SDValue Addr, SDValue &R1, SDValue &R2) {
...
bool T8xxDAGToDAGISel::tryInlineAsm(SDNode *N){
  ...
  for(unsigned i = 0, e = N->getGluedNode() ? NumOps - 1 : NumOps; i < e; ++i) {
    ...
    if ((!IsTiedToChangedOp && (!HasRC || RC != T8::IntRegsRegClassID))
        || NumRegs != 2)
      continue;
    ...
    if (Kind == InlineAsm::Kind_RegDef ||
      ...
      Register GPVR = MRI.createVirtualRegister(&T8::IntPairRegClass);
      ...
      SDValue Sub0 = CurDAG->getTargetExtractSubreg(T8::sub_even, dl, MVT::i32,
                                                    RegCopy);
      SDValue Sub1 = CurDAG->getTargetExtractSubreg(T8::sub_odd, dl, MVT::i32,
                                                    RegCopy);
      ...
      SDValue Pair = SDValue(
          CurDAG->getMachineNode(
              TargetOpcode::REG_SEQUENCE, dl, MVT::v2i32,
              {
                  ...
                  T1,
                  CurDAG->getTargetConstant(T8::sub_odd, dl, MVT::i32),
              }),
          0);
    ...
    if(PairedReg.getNode()) {
      ...
      if (IsTiedToChangedOp)
        ...
        Flag = InlineAsm::getFlagWordForRegClass(Flag, T8::IntPairRegClassID);
...
void T8xxDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::UDIV: {
    ...
    if (N->getOpcode() == ISD::SDIV) {
      TopPart = SDValue(CurDAG->getMachineNode(T8::SRAri, dl, MVT::i32, DivLHS,
                                   CurDAG->getTargetConstant(31, dl, MVT::i32)),
                        0);
    } else {
      TopPart = CurDAG->getRegister(T8::G0, MVT::i32);
    }
    TopPart = CurDAG->getCopyToReg(CurDAG->getEntryNode(), dl, T8::Y, TopPart,
                                   SDValue())
                  .getValue(1);

    // FIXME: Handle div by immediate.
    unsigned Opcode = N->getOpcode() == ISD::SDIV ? T8::SDIVrr : T8::UDIVrr;
...
/// inline asm expressions.
bool
T8xxDAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
                                                unsigned ConstraintID,
