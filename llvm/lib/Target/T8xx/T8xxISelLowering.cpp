...
static bool CC_T8xx_Assign_Split_64(unsigned &ValNo, MVT &ValVT,
                                     MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                     ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
  static const MCPhysReg RegList[] = {
    T8::I0, T8::I1, T8::I2, T8::I3, T8::I4, T8::I5
  };
...
static bool CC_T8xx_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                         MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                         ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
  static const MCPhysReg RegList[] = {
...
static bool Analyze_CC_T8xx64_Full(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                    MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                    ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool Analyze_CC_T8xx64_Half(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                    MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                    ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  ...
  if (LocVT == MVT::f32 && Offset < 16*8) {
    ...
    State.addLoc(CCValAssign::getReg(ValNo, ValVT, T8::F0 + Offset/4,
                                     LocVT, LocInfo));
    return true;
  }
...
static bool CC_T8xx64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                            CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_T8xx64_Full(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool CC_T8xx64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                            CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_T8xx64_Half(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool RetCC_T8xx64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                               CCValAssign::LocInfo &LocInfo,
                               ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_T8xx64_Full(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool RetCC_T8xx64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                               CCValAssign::LocInfo &LocInfo,
                               ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_T8xx64_Half(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}
...
static unsigned toCallerWindow(unsigned Reg) {
  static_assert(T8::I0 + 7 == T8::I7 && T8::O0 + 7 == T8::O7,
                "Unexpected enum");
  if (Reg >= T8::I0 && Reg <= T8::I7)
    return Reg - T8::I0 + T8::O0;
  return Reg;
}
...
bool T8xxTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, Subtarget->is64Bit() ? RetCC_T8xx64
                                                       : RetCC_T8xx32);
}
...
T8xxTargetLowering::LowerReturn_32(SDValue Chain, CallingConv::ID CallConv,
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    T8xxMachineFunctionInfo *SFI = MF.getInfo<T8xxMachineFunctionInfo>();
    Register Reg = SFI->getSRetReturnReg();
    if (!Reg)
      llvm_unreachable("sret virtual register not created in the entry block");
    auto PtrVT = getPointerTy(DAG.getDataLayout());
    SDValue Val = DAG.getCopyFromReg(Chain, DL, Reg, PtrVT);
    Chain = DAG.getCopyToReg(Chain, DL, T8::I0, Val, Flag);
    Flag = Chain.getValue(1);
    RetOps.push_back(DAG.getRegister(T8::I0, PtrVT));
...
SDValue
T8xxTargetLowering::LowerReturn_64(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    const SDLoc &DL, SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_T8xx64);
...
SDValue T8xxTargetLowering::LowerFormalArguments_32(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++InIdx) {
    ...
    if (VA.isRegLoc()) {
      if (VA.needsCustom()) {
        ...
        Register VRegHi = RegInfo.createVirtualRegister(&T8::IntRegsRegClass);
      ...
      Register VReg = RegInfo.createVirtualRegister(&T8::IntRegsRegClass);
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    ...
    Register Reg = SFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(&T8::IntRegsRegClass);
...
SDValue T8xxTargetLowering::LowerFormalArguments_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();

  // Analyze arguments according to CC_T8xx64.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_T8xx64);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    assert(VA.isMemLoc());
    // The CC_T8xx64_Full/Half functions compute stack offsets relative to the
  ...
  for (; ArgOffset < 6*8; ArgOffset += 8) {
    Register VReg = MF.addLiveIn(T8::I0 + ArgOffset/8, &T8::I64RegsRegClass);
...
T8xxTargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
  ...
  for (unsigned i = 0, realArgIdx = 0, byvalArgIdx = 0, e = ArgLocs.size();
    ...
    if (Flags.isSRet()) {
      ...
      SDValue StackPtr = DAG.getRegister(T8::O6, MVT::i32);
...
Register T8xxTargetLowering::getRegisterByName(const char* RegName, LLT VT,
  ...
  Register Reg = StringSwitch<Register>(RegName)
    .Case("i0", T8::I0).Case("i1", T8::I1).Case("i2", T8::I2).Case("i3", T8::I3)
    .Case("i4", T8::I4).Case("i5", T8::I5).Case("i6", T8::I6).Case("i7", T8::I7)
    .Case("o0", T8::O0).Case("o1", T8::O1).Case("o2", T8::O2).Case("o3", T8::O3)
    .Case("o4", T8::O4).Case("o5", T8::O5).Case("o6", T8::O6).Case("o7", T8::O7)
    .Case("l0", T8::L0).Case("l1", T8::L1).Case("l2", T8::L2).Case("l3", T8::L3)
    .Case("l4", T8::L4).Case("l5", T8::L5).Case("l6", T8::L6).Case("l7", T8::L7)
    .Case("g0", T8::G0).Case("g1", T8::G1).Case("g2", T8::G2).Case("g3", T8::G3)
    .Case("g4", T8::G4).Case("g5", T8::G5).Case("g6", T8::G6).Case("g7", T8::G7)
    .Default(0);
...
static void fixupVariableFloatArgs(SmallVectorImpl<CCValAssign> &ArgLocs,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    Register firstReg = (ValTy == MVT::f64) ? T8::D0 : T8::Q0;
...
SDValue
T8xxTargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_T8xx64);

  CLI.IsTailCall = CLI.IsTailCall && IsEligibleForTailCallOptimization(
                                         CCInfo, CLI, DAG.getMachineFunction());

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_T8xx64 includes all arguments.
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      if (VA.needsCustom() && VA.getValVT() == MVT::f128
        ...
        unsigned Offset = 8 * (VA.getLocReg() - T8::I0);
        unsigned StackOffset = Offset + Subtarget->getStackPointerBias() + 128;
        SDValue StackPtr = DAG.getRegister(T8::O6, PtrVT);
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_T8xx64);
...
T8xxTargetLowering::T8xxTargetLowering(const TargetMachine &TM,
  ...
  if (!Subtarget->useSoftFloat()) {
    addRegisterClass(MVT::f32, &T8::FPRegsRegClass);
    addRegisterClass(MVT::f64, &T8::DFPRegsRegClass);
    addRegisterClass(MVT::f128, &T8::QFPRegsRegClass);
  }
  if (Subtarget->is64Bit()) {
    addRegisterClass(MVT::i64, &T8::I64RegsRegClass);
  } else {
    // On 32bit sparc, we define a double-register 32bit register
    // class, as well. This is modeled in LLVM as a 2-vector of i32.
    addRegisterClass(MVT::v2i32, &T8::IntPairRegClass);
  ...
  // T8xx doesn't have i1 sign extending load
  ...
  // T8xx has no REM or DIVREM operations.
  ...
  // T8xx has no select or setcc: expand to SELECT_CC.
  ...
  // T8xx doesn't have BRCOND either, it has BR_CC.
  ...
  // ATOMICs.
  // Atomics are supported on T8xxV9. 32-bit atomics are also
  // supported by some Leon T8xxV8 variants. Otherwise, atomics
  // are unsupported.
  if (Subtarget->isV9())
  ...
  if (!Subtarget->isV9()) {
    // T8xxV8 does not have FNEGD and FABSD.
  ...
  setStackPointerRegisterToSaveRestore(T8::O6);
...
SDValue T8xxTargetLowering::LowerGlobalTLSAddress(SDValue Op,
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                     ? T8xxMCExpr::VK_T8xx_TLS_GD_HI22
                     : T8xxMCExpr::VK_T8xx_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                     ? T8xxMCExpr::VK_T8xx_TLS_GD_LO10
                     : T8xxMCExpr::VK_T8xx_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                      ? T8xxMCExpr::VK_T8xx_TLS_GD_ADD
                      : T8xxMCExpr::VK_T8xx_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                       ? T8xxMCExpr::VK_T8xx_TLS_GD_CALL
                       : T8xxMCExpr::VK_T8xx_TLS_LDM_CALL);
    ...
    Chain = DAG.getCopyToReg(Chain, DL, T8::O0, Argument, InFlag);
    InFlag = Chain.getValue(1);
    ...
    InFlag = Chain.getValue(1);
    SDValue Ret = DAG.getCopyFromReg(Chain, DL, T8::O0, PtrVT, InFlag);

    if (model != TLSModel::LocalDynamic)
      return Ret;

    SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                 withTargetFlags(Op, T8xxMCExpr::VK_T8xx_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                 withTargetFlags(Op, T8xxMCExpr::VK_T8xx_TLS_LDO_LOX10, DAG));
    HiLo =  DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
                   withTargetFlags(Op, T8xxMCExpr::VK_T8xx_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF     = ((PtrVT == MVT::i64)? T8xxMCExpr::VK_T8xx_TLS_IE_LDX
                         : T8xxMCExpr::VK_T8xx_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op,
                               T8xxMCExpr::VK_T8xx_TLS_IE_HI22,
                               T8xxMCExpr::VK_T8xx_TLS_IE_LO10, DAG);
    SDValue Ptr = DAG.getNode(ISD::ADD, DL, PtrVT, Base, TGA);
    SDValue Offset = DAG.getNode(SPISD::TLS_LD,
                                 DL, PtrVT, Ptr,
                                 withTargetFlags(Op, ldTF, DAG));
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT,
                       DAG.getRegister(T8::G7, PtrVT), Offset,
                       withTargetFlags(Op,
                                       T8xxMCExpr::VK_T8xx_TLS_IE_ADD, DAG));
  }

  assert(model == TLSModel::LocalExec);
  SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                  withTargetFlags(Op, T8xxMCExpr::VK_T8xx_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                  withTargetFlags(Op, T8xxMCExpr::VK_T8xx_TLS_LE_LOX10, DAG));
...
SDValue T8xxTargetLowering::LowerF128_LibCallArg(SDValue Chain,
...
SDValue
T8xxTargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
SDValue T8xxTargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
...
static SDValue LowerBR_CC(SDValue Op, SelectionDAG &DAG,
                          const T8xxTargetLowering &TLI, bool hasHardQuad,
                          bool isV9) {
...
static SDValue LowerVASTART(SDValue Op, SelectionDAG &DAG,
  ...
  SDLoc DL(Op);
  SDValue Offset =
      DAG.getNode(ISD::ADD, DL, PtrVT, DAG.getRegister(T8::I6, PtrVT),
...
static SDValue LowerDYNAMIC_STACKALLOC(SDValue Op, SelectionDAG &DAG,
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On T8xx32, the size of the spill area is 92. Unfortunately,
...
static SDValue LowerRETURNADDR(SDValue Op, SelectionDAG &DAG,
  ...
  if (depth == 0) {
    auto PtrVT = TLI.getPointerTy(DAG.getDataLayout());
    Register RetReg = MF.addLiveIn(T8::I7, TLI.getRegClassFor(PtrVT));
...
static SDValue LowerF64Op(SDValue SrcReg64, const SDLoc &dl, SelectionDAG &DAG,
  ...
  SDValue Hi32 = DAG.getTargetExtractSubreg(T8::sub_even, dl, MVT::f32,
                                            SrcReg64);
  SDValue Lo32 = DAG.getTargetExtractSubreg(T8::sub_odd, dl, MVT::f32,
                                            SrcReg64);
  ...
  SDValue DstReg64 = SDValue(DAG.getMachineNode(TargetOpcode::IMPLICIT_DEF,
                                                dl, MVT::f64), 0);
  DstReg64 = DAG.getTargetInsertSubreg(T8::sub_even, dl, MVT::f64,
                                       DstReg64, Hi32);
  DstReg64 = DAG.getTargetInsertSubreg(T8::sub_odd, dl, MVT::f64,
                                       DstReg64, Lo32);
  return DstReg64;
}
...
static SDValue LowerF128Load(SDValue Op, SelectionDAG &DAG)
{
  ...
  SDValue SubRegEven = DAG.getTargetConstant(T8::sub_even64, dl, MVT::i32);
  SDValue SubRegOdd  = DAG.getTargetConstant(T8::sub_odd64, dl, MVT::i32);
...
static SDValue LowerFNEGorFABS(SDValue Op, SelectionDAG &DAG, bool isV9) {
  ...
  SDValue Hi64 = DAG.getTargetExtractSubreg(T8::sub_even64, dl, MVT::f64,
                                            SrcReg128);
  SDValue Lo64 = DAG.getTargetExtractSubreg(T8::sub_odd64, dl, MVT::f64,
                                            SrcReg128);
  ...
  SDValue DstReg128 = SDValue(DAG.getMachineNode(TargetOpcode::IMPLICIT_DEF,
                                                 dl, MVT::f128), 0);
  DstReg128 = DAG.getTargetInsertSubreg(T8::sub_even64, dl, MVT::f128,
                                        DstReg128, Hi64);
  DstReg128 = DAG.getTargetInsertSubreg(T8::sub_odd64, dl, MVT::f128,
                                        DstReg128, Lo64);
  return DstReg128;
}
...
SDValue T8xxTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
  ...
  case Intrinsic::thread_pointer: {
    EVT PtrVT = getPointerTy(DAG.getDataLayout());
    return DAG.getRegister(T8::G7, PtrVT);
  }
  }
}

SDValue T8xxTargetLowering::
...
SDValue T8xxTargetLowering::bitcastConstantFPToInt(ConstantFPSDNode *C,
                                                    const SDLoc &DL,
                                                    SelectionDAG &DAG) const {
...
SDValue T8xxTargetLowering::PerformBITCASTCombine(SDNode *N,
...
SDValue T8xxTargetLowering::PerformDAGCombine(SDNode *N,
                                               DAGCombinerInfo &DCI) const {
  switch (N->getOpcode()) {
  default:
    break;
...
T8xxTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case T8::SELECT_CC_Int_ICC:
  case T8::SELECT_CC_FP_ICC:
  case T8::SELECT_CC_DFP_ICC:
  case T8::SELECT_CC_QFP_ICC:
    if (Subtarget->isV9())
      return expandSelectCC(MI, BB, T8::BPICC);
    return expandSelectCC(MI, BB, T8::BCOND);
  case T8::SELECT_CC_Int_XCC:
  case T8::SELECT_CC_FP_XCC:
  case T8::SELECT_CC_DFP_XCC:
  case T8::SELECT_CC_QFP_XCC:
    return expandSelectCC(MI, BB, T8::BPXCC);
  case T8::SELECT_CC_Int_FCC:
  case T8::SELECT_CC_FP_FCC:
  case T8::SELECT_CC_DFP_FCC:
  case T8::SELECT_CC_QFP_FCC:
    if (Subtarget->isV9())
      return expandSelectCC(MI, BB, T8::FBCOND_V9);
    return expandSelectCC(MI, BB, T8::FBCOND);
  }
}

MachineBasicBlock *
T8xxTargetLowering::expandSelectCC(MachineInstr &MI, MachineBasicBlock *BB,
  ...
  BuildMI(*SinkMBB, SinkMBB->begin(), dl, TII.get(T8::PHI),
...
T8xxTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':
      if (VT == MVT::v2i32)
        return std::make_pair(0U, &T8::IntPairRegClass);
      else if (Subtarget->is64Bit())
        return std::make_pair(0U, &T8::I64RegsRegClass);
      else
        return std::make_pair(0U, &T8::IntRegsRegClass);
    case 'f':
      if (VT == MVT::f32 || VT == MVT::i32)
        return std::make_pair(0U, &T8::FPRegsRegClass);
      else if (VT == MVT::f64 || VT == MVT::i64)
        return std::make_pair(0U, &T8::LowDFPRegsRegClass);
      else if (VT == MVT::f128)
        return std::make_pair(0U, &T8::LowQFPRegsRegClass);
    ...
    case 'e':
      if (VT == MVT::f32 || VT == MVT::i32)
        ...
        return std::make_pair(0U, &T8::DFPRegsRegClass);
      else if (VT == MVT::f128)
        return std::make_pair(0U, &T8::QFPRegsRegClass);
      // This will generate an error message
      return std::make_pair(0U, nullptr);
    }
  }
  ...
  if (Subtarget->is64Bit() && VT == MVT::i64) {
    assert(ResultPair.second == &T8::IntRegsRegClass &&
           "Unexpected register class");
    return std::make_pair(ResultPair.first, &T8::I64RegsRegClass);
  }
...
void T8xxTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::READCYCLECOUNTER: {
    ...
    SDValue Lo = DAG.getCopyFromReg(N->getOperand(0), dl, T8::ASR23, MVT::i32);
    SDValue Hi = DAG.getCopyFromReg(Lo, dl, T8::G0, MVT::i32);
    SDValue Ops[] = { Lo, Hi };
...
bool T8xxTargetLowering::useLoadStackGuardNode() const {
...
void T8xxTargetLowering::insertSSPDeclarations(Module &M) const {
  if (!Subtarget->isTargetLinux())
