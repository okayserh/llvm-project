...
T8xxDebugValueManager::T8xxDebugValueManager(MachineInstr *Def)
    : Def(Def) {
...
static bool isSameScalarConst(const MachineInstr *A, const MachineInstr *B) {
  ...
  if (A->getOpcode() != B->getOpcode() ||
      !T8xx::isScalarConst(A->getOpcode()) ||
      !T8xx::isScalarConst(B->getOpcode()))
    return false;
...
T8xxDebugValueManager::getSinkableDebugValues(
  ...
  for (auto *DV : DbgValues) {
    ...
    /* OKH, See what this is used for
    if (!T8xx::isScalarConst(Def->getOpcode()))
      continue;
    */
...
bool T8xxDebugValueManager::isInsertSamePlace(
    MachineInstr *Insert) const {
  if (Def->getParent() != Insert->getParent())
    return false;
  for (MachineBasicBlock::iterator MI = std::next(Def->getIterator()),
...
void T8xxDebugValueManager::sink(MachineInstr *Insert) {
...
void T8xxDebugValueManager::cloneSink(MachineInstr *Insert,
                                             Register NewReg,
                                             bool CloneDef) const {
  MachineBasicBlock *MBB = Insert->getParent();
  MachineFunction *MF = MBB->getParent();
...
void T8xxDebugValueManager::updateReg(Register Reg) {
...
void T8xxDebugValueManager::replaceWithLocal(unsigned LocalId) {
  /* TODO: OKH, Check what it's good for
  for (auto *DBI : DbgValues) {
    auto IndexType = DBI->isIndirectDebugValue()
                         ? llvm::T8xx::TI_LOCAL_INDIRECT
                         : llvm::T8xx::TI_LOCAL;
    for (auto &MO : DBI->getDebugOperandsForReg(CurrentReg))
...
void T8xxDebugValueManager::removeDef() {
